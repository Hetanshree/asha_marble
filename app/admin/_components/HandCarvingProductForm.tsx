...
import type { Availability, HandCarvingProduct } from "../_lib/types";
import { Banner, Button, Card, Field, Input, Select, Textarea, Toggle } from "./ui";

type UploadedImage = { url: string; publicId: string };

export default function HandCarvingProductForm({ initialProduct }: { initialProduct?: HandCarvingProduct }) {
  ...
  async function removeImage(image: UploadedImage) {
    ...
    try {
      if (isEdit && originalPublicIds.has(image.publicId)) {
        await api.delete(`/api/hand-carving/products/${initialProduct!._id}/images`, { publicId: image.publicId });
      } else {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      if (isEdit) {
        await api.put(`/api/hand-carving/products/${initialProduct!._id}`, payload);
      } else {
        await api.post("/api/hand-carving/products", payload);
      }
      router.push("/admin/hand-carving/products");
      router.refresh();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Field label="Category" htmlFor="category" error={fieldErrors.category?.[0]}>
            <Input id="category" required placeholder="e.g. Carved Jali" value={category} onChange={(e) => setCategory(e.target.value)} />
          </Field>
          <Field label="Material" htmlFor="material">
            <Input id="material" placeholder="e.g. Jaisalmer Yellow Marble" value={material} onChange={(e) => setMaterial(e.target.value)} />
          </Field>
          <Field label="Dimensions / Size" htmlFor="dimensions">
            <Input id="dimensions" placeholder="e.g. 24in x 36in x 1.5in" value={dimensions} onChange={(e) => setDimensions(e.target.value)} />
          </Field>
          <Field label="Price" htmlFor="price" error={fieldErrors.price?.[0]}>
            <Input id="price" required placeholder="e.g. ₹4,500 or On Request" value={price} onChange={(e) => setPrice(e.target.value)} />
          </Field>
          <Field label="Unit" htmlFor="unit" error={fieldErrors.unit?.[0]}>
            <Input id="unit" required placeholder="e.g. per piece" value={unit} onChange={(e) => setUnit(e.target.value)} />
          </Field>
          ...
          {images.map((image, index) => (
            <div key={image.publicId || image.url} className="group relative h-24 w-24 overflow-hidden rounded-md border border-neutral-200">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 bg-black/50 opacity-0 transition group-hover:opacity-100">
                ...
                <button type="button" onClick={() => removeImage(image)} className="rounded bg-red-600 px-1.5 text-xs text-white">
                  Remove
                </button>
              </div>
            </div>
      ...
      <div className="flex gap-3">
        ...
        <Button type="button" variant="ghost" onClick={() => router.push("/admin/hand-carving/products")}>
          Cancel
        </Button>
      </div>
    </form>
