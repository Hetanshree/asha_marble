...
import type { HandCarvingProduct, Pagination } from "../../../_lib/types";
import { Banner, Button, Card, Input, PageHeader, Spinner, Toggle } from "../../../_components/ui";

export default function HandCarvingProductsListPage() {
  const [products, setProducts] = useState<HandCarvingProduct[]>([]);
  ...
  useEffect(() => {
    ...
    api
      .get<HandCarvingProduct[]>(`/api/hand-carving/products?${params.toString()}`)
      .then((res) => {
  ...
  async function toggleActive(product: HandCarvingProduct) {
    try {
      await api.put(`/api/hand-carving/products/${product._id}`, { active: !product.active });
  ...
  async function remove(product: HandCarvingProduct) {
    if (!confirm(`Delete "${product.name}"? This also removes its images from Cloudinary.`)) return;
    try {
      await api.delete(`/api/hand-carving/products/${product._id}`);
  ...
  return (
    <div>
      <PageHeader
        title="Carving Products"
        action={
          <Link href="/admin/hand-carving/products/new">
      ...
      <Card className="mt-4 p-0">
        {loading ? (
          ...
          <p className="p-6 text-sm text-neutral-500">No carving products found.</p>
        ) : (
                  ...
                  <tr key={product._id}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex justify-end gap-2">
                        <Link href={`/admin/hand-carving/products/${product._id}`}>
